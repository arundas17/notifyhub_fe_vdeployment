...
const FollowerCard = () => {
  const { departments, setDepartmentSearch, loading, error }: any = useContext(UserDataContext);
  ...
  const renderContent = () => {
    if (loading) {
      return <p className="text-center col-span-12">Loading departments...</p>;
    }

    if (error) {
      return <p className="text-center col-span-12">Error fetching departments: {error.message}</p>;
    }

    if (!departments || departments.length === 0) {
      return <p className="text-center col-span-12">No departments found.</p>;
    }
