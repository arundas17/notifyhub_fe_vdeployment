...
export const userService = {
  ...
  async getMe() {
    ...
    const result = await client.query<{ user: { id: string; username: string; email: string; company: { id: string; name: string } } }>({ query, variables: { id: "1" } });
  ...
  async getAllUsers() {
    ...
    `;
    try {
      const result = await client.query<{ users: { id: string; username: string; email: string; company: { id: string; name: string } }[] }>({ query });
